...
class OrderRequest {
  ...
  OrderRequest({required this.symbol, required this.entryPrice, required this.stopLoss});
  Map<String, dynamic> toJson() => {
    'symbol': symbol,
...
class AppSettings {
  ...
  AppSettings({required this.tradeAmount, required this.rangeValue, required this.profitPercent});
...
class TradeOrder {
  ...
  TradeOrder({
    this.id, required this.symbol, this.qty, this.status,
    this.takeProfit, this.stopLoss, this.createdAt, this.side, this.filledAvgPrice,
  });

  factory TradeOrder.fromJson(Map<String, dynamic> j) => TradeOrder(
    id: j['id']?.toString() ?? j['clientOrderId']?.toString(),
    symbol: j['symbol'] ?? '',
...
class Account {
  ...
  Account({
    required this.equity, required this.cash, required this.portfolioValue,
    required this.buyingPower, required this.daytradeCount, required this.status,
  });
...
class AppUser {
  ...
  AppUser({required this.id, required this.username, required this.role, required this.active});
